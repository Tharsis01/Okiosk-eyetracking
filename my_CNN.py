...
# class ConvNet(torch.nn.Module):# main_y
#     def __init__(self):
...
# class ConvNet(torch.nn.Module):# sub_x
#     def __init__(self):
#         super(ConvNet, self).__init__()
...
class ConvNet(torch.nn.Module): # main_x
    def __init__(self):
        super(ConvNet, self).__init__()
...
def dataLoad(path, want = 0):
    ...
    for name in nameList:
        ...
        totalHolder.append(((torch.tensor(np.array([[im]])).to(dtype=torch.float,device=device))/top,
...
trainingSet = dataLoad("LRtrain")
test = dataLoad("LRtest")
...
def trainModel():
    ...
    for epoch in range(num_epochs):
        ...
        for i,(im, label) in enumerate(trainingSet):
            ...
            if (i+1) % 1000 == 0:
                # testSc = evaluateModel(model,test,sidelen=1920)
                testSc = evaluateModel(model,test)
                # trainSc = evaluateModel(model,trainingSet,sidelen=1920)
    ...
    # finalScore = evaluateModel(bestModel,test,sidelen=1920)
    print(finalScore)

    # if finalScore < 1500:
    torch.save(bestModel.state_dict(), "my_xModels/" + str(int(finalScore))+".plt")
